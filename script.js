...
function startCountdown() {
  ...
  // Parse as LOCAL midnight (not UTC) — fixes timezone bug
  ...
  // BUG FIX 1: Show the countdown section (was permanently display:none)
  ...
  // BUG FIX 2: Show cancel button
  document.getElementById("cancelBtn").style.display = "inline-flex";

  // Start ticking immediately
  runLocalTimer(eventDate, eventName);

  // Tell backend to schedule emails (non-blocking)
...
function runLocalTimer(eventDate, eventName) {
  activeTimer = setInterval(function () {
    ...
    if (distance <= 0) {
      ...
      // BUG FIX 3: hide countdown blocks, show started banner correctly
    ...
    // BUG FIX 4: update individual span elements, NOT innerHTML of whole section
...
function cancelCountdown() {
  ...
  // BUG FIX 5: correct element IDs (old code used #countdown which doesn't exist)
  document.getElementById("countdownDisplay").style.display = "none";
  document.getElementById("cancelBtn").style.display        = "none";
